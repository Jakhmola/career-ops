...
try {
  ...
  // Off-domain HTTPS is ALLOWED: tenants with branded careers domains
  // (Tether → careers.tether.io, Hostaway → careers.hostaway.com) put them in
  // careers_url, and blanking those URLs killed the offers at the scanner's
  // invalid-URL guard. Only non-https and missing URLs blank now; navigation
  // safety lives in the liveness layer's private-host guard.
...
try {
  ...
  const LIGATURE_TEMPLATES = [
    'cv-template.html',
    'cover-letter-local.html',
    // resume-template.html removed locally 2026-07-04 (single CV template policy).
    // cover-letter-template.html removed locally (bullet cover path deleted).
  ];
...
try {
  ...
  // resume-template.html removed locally 2026-07-04: cv-template.html is the
  // single CV template (single-template policy, modes/_custom.md).
} catch (e) {
