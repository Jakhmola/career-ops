...
async function checkPlaywright() {
  ...
  // present but no binary — just ABOUT + LICENSE files). When the bundled Chromium
  // download is broken, browser-exec.mjs transparently substitutes a system Chrome
  // (the same fallback every scanner uses), so a healthy system browser passes too.
  let browser;
  try {
    const { chromiumLaunchOptions, resolveBrowserExecutable } = await import('./browser-exec.mjs');
    ...
    return {
      pass: true,
      label: systemBrowser
        ? `Playwright using system browser (${systemBrowser})`
