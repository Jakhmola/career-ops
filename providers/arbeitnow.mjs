...
// override with `max_pages`).
//
// Local wiring: scan.mjs reaches this through the `aggregators:` block in
// portals.yml (fetch receives a descriptor whose `config` holds the YAML entry).
// Aggregators export NO detect() — the per-company resolveProvider() loop must
...
/** Resolve the page cap: a positive integer `max_pages`, capped. Accepts both
 *  the local aggregator-descriptor shape (entry.config.max_pages) and a flat
 *  entry (entry.max_pages). */
function resolveMaxPages(entry) {
  const v = entry?.config?.max_pages ?? entry?.max_pages;
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    for (let page = 1; page <= maxPages; page++) {
      ...
      ctx.recordCall?.('arbeitnow'); // local: aggregator calls feed the api-usage ledger
